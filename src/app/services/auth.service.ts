...
export class AuthService {
  ...
  completeGoogleRegistration(data: { uid: string; email: string; username: string; photoURL?: string; displayName?: string }): Observable<User> {
    const newUser: User = {
      id: data.uid,
      email: data.email,
    ...
    return this.firebaseAuthService.signInWithGoogle().pipe(
      ...
      catchError(err => {
        if (err.message === 'USER_NOT_REGISTERED') {
          throw new Error('Tài khoản chưa được đăng ký. Vui lòng đăng ký trước.');
        }
        throw err;
      })
    );
  ...
  private validateLogin(loginData: LoginRequest): User {
    ...
    const user = users.find(u => 
      (u.email === loginData.usernameOrEmail || u.username === loginData.usernameOrEmail)
    );
